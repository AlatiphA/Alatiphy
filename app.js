...
async function silentCacheSong(song) {
  ...
  try {
    ...
    await saveSongToDB({
      ...
      isRealDownload: false
    });

    await loadDownloadedSongsMap();

    // no indicator/UI refresh needed — silent cache stays invisible
...
function isDownloaded(index) {
  ...
  const entry = STATE.downloadedSongs[song.src];
...
DOM.menuDownload.onclick = async () => {
  ...
  try {
    ...
    // already a REAL download?
    const existing = await getSongFromDB(song.src);

    if (existing && existing.isRealDownload) {
    ...
    // reuse silently cached blob if we already have it — avoids re-fetching
    ...
    // SAVE / UPGRADE TO REAL DOWNLOAD IN INDEXEDDB
    await saveSongToDB({
    ...
    // SAVE ACTUAL FILE TO DEVICE DOWNLOADS FOLDER
    saveBlobToDeviceDownloads(blob, song.title);
...
/* ===== SAVE BLOB AS REAL FILE TO DEVICE DOWNLOADS ===== */
function saveBlobToDeviceDownloads(blob, title) {
  ...
  try {
    ...
    console.error("Failed to save file to downloads", err);
  }
}
